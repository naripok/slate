...
function configure(pkg, env, target) {
  ...
  const input = `packages/${pkg.name.replace(/@naripok\//, '')}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    // modules by default.
    commonjs({
      exclude: [`packages/${pkg.name.replace(/@naripok\//, '')}/src/**`],
    ...
    babel({
      runtimeHelpers: true,
      include: [`packages/${pkg.name.replace(/@naripok\//, '')}/src/**`],
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkg.name.replace(/@naripok\//, '')}/${
          isProd ? pkg.umdMin : pkg.umd
        }`,
        exports: 'named',
        name: startCase(pkg.name.replace(/@naripok\//, '')).replace(/ /g, ''),
  ...
  if (isCommonJs) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.name.replace(/@naripok\//, '')}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.name.replace(/@naripok\//, '')}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
      ],
